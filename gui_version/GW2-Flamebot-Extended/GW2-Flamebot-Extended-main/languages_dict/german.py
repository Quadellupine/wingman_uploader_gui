...
german = {
    ...
    "VG MVP BLEU S": "* {mvp_names} wurde **{max_bleu}** Mal nach Narnia teleportiert.]*",
    "VG MVP BLEU P": "* {mvp_names} wurdet **{max_bleu}** Mal nach Narnia teleportiert.]*",
    
    "LVP VG RED": "* {lvp_names} Danke, dass du/ihr **Red Guardian** getötet hast/habt. Lass dir/euch von niemandem erzählen, dass **VG** kein Condi Boss ist.]*",
    
    
    ################################ GORS ################################
    
    "GORS MVP SPLIT": "* {mvp_names} mit nur **{dmg_ratio:.1f}%** Schaden bei **Split** als **DPS**]*",
    
    "GORS MVP EGG S": "* **chirp chirp** {mvp_names} kam in P3 aus dem **Ei**]*",
    "GORS MVP EGG P": "* **chirp chirp** {mvp_names} kamen in P3 aus dem **Ei**]*",
    
    "MVP GORS SLAM": "* Dink donk {mvp_names}! Warum lässt du zu, dass alle durch **Gorseval Slam** Cancer bekommen? Entschuldige dich sofort bei {cucked_players}.]*",
    
    "GORS LVP SPLIT": "* {lvp_names} mit **{dmg_ratio:.1f}%** Schaden bei **Split** als **DPS**]*",
    
    
    ################################ SABETHA ################################
    
    "SABETHA MVP SPLIT": "* {mvp_names} mit nur **{dmg_ratio:.1f}%** Schaden bei **Split** ohne **Kanonen** zu machen]*",
    
    "SABETHA MVP BOMB" : "* {mvp_names}, der/die eine Bombe auf seinen/ihren **Trupp explodieren** ließ]*",
    
    "MVP SABETHA FLAMEWALL" : "* Huch! {mvp_names} sind in die **Flamewall** gelaufen und haben Selbstmord begangen.]*",
    
    "SABETHA LVP SPLIT": "* {lvp_names} mit **{dmg_ratio:.1f}%** Schaden bei **Split** als **DPS**]*",
    
    "LVP SABETHA CANNON": "* {lvp_names} Vielen Dank für die **Kanonen**. Diese anderen AFK **DPS** Spieler könnten das nie.]*",
    
    
    ################################ SLOTH ################################
    
    "SLOTH MVP 0 CC S"   : "* {mvp_names}, der/die sich nicht einmal die Mühe gemacht hat, **CC** zu machen, ohne **Shroom** zu essen.]*",
    "SLOTH MVP 0 CC P"   : "* {mvp_names}, die sich nicht einmal die Mühe gemacht haben, **CC** zu machen, ohne **Shroom** zu essen.]*",
    "SLOTH MVP CC S"     : "* {mvp_names}, der/die nur **{min_cc:.0f}** **CC** (**{cc_ratio:.1f}%** des Trupps) gemacht hat, ohne **Shroom** zu essen.]*",
    "SLOTH MVP CC P"     : "* {mvp_names}, die nur **{min_cc:.0f}** **CC** (**{cc_ratio:.1f}%** des Trupps) gemacht haben, ohne **Shroom** zu essen.]*",
    
    "SLOTH MVP TANTRUM S": "* {mvp_names} hat **{max_tantrum}** mal den **Wutanfall** ausgehalten, steh auf, Champ]*",
    "SLOTH MVP TANTRUM P": "* {mvp_names} haben **{max_tantrum}** mal den **Wutanfall** ausgehalten, steh auf, Champs]*",
    
    "LVP SLOTH SHROOM": "* {lvp_names} haben **Pilze** gegessen und hinterließ keine Krümel.]*",
    
    "LVP SLOTH KEV": "* Blut für die Blutgötter! **{lvp_names}** wurde brutal ermordet! Oh, welch sinnlose Gewalt!]*",
    
    "LVP SLOTH KEV ALIVE": "* Was? **{lvp_names}** hat überlebt?? Was ist das, ein opfer für die Pro-Life-Bewegung???]*",
    ...
    "KC MVP ORB"  : "* {mvp_names} gave **{min_orb} orbs** to the **boss**]*",
    ...
    "DEIMOS MVP BLACK S": "* {mvp_names} Kudos to this **champion** for stepping in **{max_black} oils**]*",
    "DEIMOS MVP BLACK P": "* {mvp_names} Kudos to theese **champions** all of them {nb_players} triggered an **{max_black} oil**]*",
